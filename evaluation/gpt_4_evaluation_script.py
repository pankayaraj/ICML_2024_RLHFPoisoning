...
dataset = load_from_disk("EVALUATION DATASET LOCATION") #this will be generated by running datasets/poiosin/poison_random.py. 
...
for i in range(200):
    ...
    response = poiosned_reponse[i].replace(token, "").replace("<s>", "").replace(",", "").strip()

    query  = STRING_FORMAT.format(prompt=prompt, response=response)
    gpt_reponse = get_refined_caption(gpt_reponse)

torch.save(scores, "GPT4 Score Save Location")